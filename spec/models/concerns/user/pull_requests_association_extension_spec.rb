  ...
  describe '#assigned_pull_requests.not_referenced_by_other_users' do
    context 'ユーザーがアサインしている PullRequest の内 (has_may :assigned_pull_requests)' do
      ...
      it '他のユーザーがアサインしていない PullRequest を返すこと' do
        taro.assigned_pull_requests << create(:pull_request, repository:, id: 100)
        taro.assigned_pull_requests << create(:pull_request, repository:, id: 200) { |pr| pr.assignees << jiro }
      ...
      it '他のユーザーがレビューしていない PullRequest を返すこと' do
        taro.assigned_pull_requests << create(:pull_request, repository:, id: 100)
        taro.assigned_pull_requests << create(:pull_request, repository:, id: 200) { |pr| pr.reviewers << jiro }
